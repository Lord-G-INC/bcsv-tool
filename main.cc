...
void HandleBCSV(const ghc::filesystem::path& arg, const ghc::filesystem::path& dir) {
    Reader reader{arg.string().c_str(), (dir / "lookup_supermariogalaxy.txt").string().c_str()};
    ghc::filesystem::path out{arg};
    reader.WriteCSV(out.replace_extension(".csv").string().c_str());
}

void HandleCSV(const ghc::filesystem::path& arg) {
